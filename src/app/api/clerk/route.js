...
export async function POST(req) {
  const WEBHOOK_SECRET = process.env.WEB_HOOK_SIGNING_SECRET;

  if (!WEBHOOK_SECRET) {
    throw new Error(
      "Please add WEB_HOOK_SIGNING_SECRET from Clerk Dashboard to .env or .env.local"
    );
  }
  ...
  if (eventType === "user.deleted") {
    try {
      await deleteUser(id); // Pass id directly
    } catch {
